...
var PATTERN_SLASH = /\/?$/,
	...
	combo = pegasus.createPipe({
		...
		fit: function (request, response) {
			...
			if (config.source && response.status() === 404) {
		...
		main: function (request, response) {
			...
			new FileManager({
				...
				}).require(url.pathnames, url.params.aio, function (err, files) {
				if (err) {
					if (err.code === 'ENOENT' || err.code === 'EVERCONF') {
						// File not found or version conflicts.
						util.error(err);

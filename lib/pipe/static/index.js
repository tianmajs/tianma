...
var PATTERN_PATHNAME_END = /\/?$/,
	...
	static = pegasus.createPipe({
		 ...
		 */
		_done: function (status, data, contextType) {
		...
		_readFile: function (filename) {
			...
			fs.readFile(filename, function (err, data) {
				if (err) {
					...
					self._done(200, data, util.mime(filename));
				}
			});
		},
		...
		main: function (request, response) {
			var config = this._config,
				context = this.context,
				filename = getFilename(request.pathname, context.base, config.root),
				self = this;
